...
def _merkmale_prompt(blaetter) -> str:
    ...
    return (
        ...
        "VORAUSSETZUNGEN (Merkmale). Ordne JEDE Voraussetzung genau EINER Zelle zu "
        "(Erfuellung + Beweismittel):\n"
        "  erfuellt_urkunde -- der Fall erfuellt sie UND nennt dafuer ein Dokument/einen "
        "Vertrag/eine Rechnung\n"
        "  erfuellt_zeuge   -- der Fall erfuellt sie UND nennt dafuer einen Zeugen\n"
        "  erfuellt_aussage -- der Fall erfuellt sie, aber nur durch die eigene Erzaehlung "
        "(kein Beleg genannt)\n"
        "  nicht_erfuellt   -- der Fall sagt ausdruecklich, dass sie NICHT vorliegt\n"
        "  offen            -- der Fall sagt dazu gar nichts\n"
        "Die Voraussetzungen:\n" + "\n".join(zeilen) + "\n"
        "Gib NUR ein kompaktes JSON-Objekt zurueck: {\"<merkmal-id>\": \"<zelle>\", ...}. "
        "Erfinde keine Voraussetzung, waehle nur aus der Liste. Du entscheidest NICHT, ob der "
        "Anspruch besteht -- nur, was die Fakten belegen.\n"
        "WICHTIG: Schildert der Fall die Voraussetzung ueberhaupt (auch nur durch Erzaehlen), "
        "dann ist sie mindestens 'erfuellt_aussage' -- NICHT 'offen'. 'offen' nur, wenn die "
        "Schilderung dazu wirklich nichts sagt.\n"
        ...
        "merkmal:faelligkeit -> {\"merkmal:angebot\": \"erfuellt_urkunde\", \"merkmal:annahme\": "
        "\"erfuellt_urkunde\", \"merkmal:faelligkeit\": \"erfuellt_aussage\"}"
    )
