...
**Rot — der nächste Schritt:** die *abgeleitete* Schwelle **billig in den heißen Pfad** verdrahten via **gespeicherter Kalibrierung** (BeliefStability-Muster: batch rechnen → als Wissen ablegen → read-time billig lesen); dann weitere Selbst-Parameter (auch die Symmetrie-Rate). *Oder* SYSTEME in die **Breite** (weitere Regel-Domänen, z. B. Grammatik). Das LLM bleibt am Rand, gedeckelt. (Optional: Sprachen en/fr.)
