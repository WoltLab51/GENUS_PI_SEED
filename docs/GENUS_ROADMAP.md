...
**Rot — der nächste Schritt:** **SYSTEME ①b** — die *gelernten* Regeln in `infer()` verdrahten (gelernt ersetzt Saat, Saat als Fallback) + Überraschung bei Regel-Verletzung; dann weitere Regel-Domänen. Das LLM bleibt am Rand, gedeckelt. (Optional offen aus Phase A: weitere Sprachen en/fr; separat: die 6 is_a-Zyklen bereinigen.)
