...
**Rot — der nächste Schritt:** der **Deuter-Benchmark** (kleines generatives Modell lokal auf dem Pi messen, wie beim Embedder — erst messen, dann bauen), dann Deuter → Stimme → Personen-Gedächtnis als eigener Design-Schritt. Parallel offen: Phase D (früh), weitere Regel-Domänen, Korrelation≠Kausalität, Ziel-Objekt. Das LLM bleibt am Rand, gedeckelt.
