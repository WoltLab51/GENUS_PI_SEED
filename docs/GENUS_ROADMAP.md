...
**Rot — der nächste Schritt:** die **Stimme** (freies, natürliches Formulieren statt Templates — die Antwortseite; im Würfel als Antwort-Komposition schon mitgedacht: eine Zelle indexiert später *beides*, wie gedeutet UND wie geantwortet wird). Parallel offen: Treffer-Quote-Kennzahl aus Folge-Signalen, Scan-Differenzierung neuer Ausprägungen, Raster-Inquiries (GENUS fragt bei Unsicherheit nach), Plural-Morphologie beim Objekt einer Beziehungsfrage, Phase D (früh), Korrelation≠Kausalität, Ziel-Objekt. Das LLM bleibt am Rand, gedeckelt.
