...
**Grün, neu (2026-07-06, DIE TIEFUNG — die neuen Objekt-Konzepte platzieren):** Nach dem Backfill waren die dynamischen Objekte (Fahrradnabe, Stute, …) INSELN: benannt, aber ohne is_a — kein Platz in der Hierarchie, keine Geschwister, kein Analogie-Anker; und der Kletter-Lerner erreicht sie nicht (er klettert is_a-Objekte, keine dynamischen Objekte). `deploy/backfill_konzepte.py --tiefung` (derselbe Ernter, DRY) erntet die **7.788 Inseln** und nimmt dabei is_a HINZU, um sie zu platzieren; der Ring-Filter ist auf is_a verallgemeinert (auch is_a ist transitiv → auch da kein Fremd-Zyklus in den Ledger). **Live gelaufen:** 7.871 Inseln → 25.381 Kanten (7.387 trugen welche, ~94% — reale Konzepte haben fast alle is_a) + 19.281 Label-Kanten; **4 reale Wikidata-Ringe abgefangen** (die is_a-Verallgemeinerung des Filters war nötig). is_a-Kanten im Ledger 15.562 → **26.117** (+68% Hierarchie-Dichte), Ledger gesamt ~262k Kanten. **Der Payoff, ehrlich gemessen bei den Inseln selbst:** von 2.000 platzierten Inseln haben **1.091 jetzt Geschwister** (vorher null) und **83 zünden eine eigene Vermutung** — neue Analogie-Anker, die vorher unmöglich waren (»Sri-Lanker ist ein Einwohner« 5/6, »Hadron ist eine chemische Substanz« 2/2). Die Zünd-Rate auf dem ALTEN Subjekt-Sample bewegte sich nur wenig (55→61) — ehrlich: der Gewinn liegt bei den neu platzierten Konzepten, nicht bei den alten; und Analogie zündet erst, wo eine Mehrheit der Geschwister eine Eigenschaft teilt (die nächste Tiefung — eine weitere Objekt-Ebene, oder used_for/causes im Generator zulassen — würde die Dichte weiter heben). 915 grün.

**Grün, neu (2026-07-06, DIE REKURSIVE TIEFUNG — und ihr ehrliches Ende):** `--rekursiv` (im selben Ernter, `_ernte()` herausgehoben) läuft die Tiefung Runde um Runde über die je NEU aufgetauchten Objekt-Schalen, HART gedeckelt (`--max-runden`, Default 3) mit ehrlicher Abbruch-Meldung. **Der Befund, der das Werkzeug formte:** die Schale schrumpft NICHT — jede Ebene ist ~8k Konzepte breit (Ebene 1: 7.788 → Ebene 2: 7.874 → Ebene 3: 8.545, danach noch 8.351 offen), die Wikidata-Nachbarschaft weitet sich konstant; „bis leer" importierte halb Wikidata, darum der Deckel. **Live 2 Ebenen gelaufen:** Ebene 2 = 24.007 Kanten + 18.437 Labels (4 Ringe), Ebene 3 = 28.618 Kanten + 18.239 Labels (5 Ringe) — Platzierungsquote STEIGT mit der Tiefe (84 → 95%, tiefe Items haben fast alle is_a). Kumulativ: Konzepte ~12k → **35.912** (≈3×), is_a 15.562 → **40.722**, Ledger ~360k Kanten. **Aber der ehrliche Payoff widerlegt die Erwartung:** die breite Zünd-Rate (festes 1500er-Sample) ging 55 → 61 → **54** — die Rekursion fügte MASSE hinzu, aber keine proportionale Analogie-Fähigkeit; die vielen abstrakten tiefen Konzepte (physikalische Größen, anatomische Klassen) verdünnen die Dichte sogar leicht. Platzierung ≠ nützliche Analogie. **Entscheidung (Ronny + Claude): Schluss nach Ebene 3** — konstante Kosten × null Dichte-Gewinn = klarer Stopp; das Werkzeug bleibt für gezielte tiefe Felder, nicht als Breitband-Import. Der wertvolle Kern (Ebenen 0–2: alles, was der Wortschatz direkt berührt) liegt. 915 grün.

**Grün, neu (2026-07-06, DER GENERATOR LERNT KAUSALITÄT — used_for/causes, ehrlich):** Ronny: „used_for/causes im Generator widerlegbar machen." Der Hypothese-Generator ließ nur is_a/part_of zu, weil eine unwiderlegbare Vermutung unehrlich ist (vakuöses „offen"). Der Befund war ASYMMETRISCH: **`causes` hat eine Inverse (`caused_by`, ~1.843 reale Kanten)** → „A verursacht B" ist ehrlich widerlegt, wenn der Graph die GEGENrichtung kennt (`_kausal_urteil`: bestätigt bei gleicher Richtung — auch als Inverse —, widerlegt bei Gegenrichtung), mit ehrlichem Rückkopplungs-Vorbehalt im Beweistext. **`used_for` hat keine Inverse** und keine saubere Azyklizität; ehrlich widerlegbar nur über eine Typ-Disjunktheits-Saat, die echte Wikidata-Kanten fehlwiderlegen würde → **bewusst DRAUSSEN gelassen** (die ehrliche Decke als Feature, nicht Makel). `ANALOGIE_PRAEDIKATE = is_a/part_of/causes/caused_by`; used_for/made_of/has_part bleiben draußen. Design über Verstehen→Entwurf-Workflow (asymmetrischer Befund bestätigt), dann **adversarialer Review, der einen echten Beweistext-Bug fand**, bevor er den Pi erreichte: der Kausal-Widerlegungs-Text nannte bei einer `caused_by`-Vermutung die falsche Seite als Ursache — gefixt an der Klasse (Ursache aus dem Prädikat abgeleitet) + Test. Selbstbild: `faehigkeit:analogie` inhaltlich um Kausal-Analogie erweitert (bleibt „teilweise"). 921 grün. Lehre (GENUS_INTELLIGENZ.md): *man darf nur dort vermuten, wo man auch falsch liegen kann — Widerlegbarkeit ist die Eintrittskarte einer Operation.* **Nachgezogen (06707b2): TRANSITIVE Kausalketten** — `_kausal_erreicht` macht einen tiefen-gedeckelten BFS über die vereinte Kausalkette (causes + caused_by-Inverse), sodass „A verursacht B" auch dann widerlegt ist, wenn B über eine KETTE B→…→A zurückführt (der Ring, egal wie lang); bestätigt bleibt bewusst direkt-only (mediierte Kausation ist kein direkter Nachweis), eine Vorwärts-Kette widerlegt die Vorwärts-Vermutung nicht. **Empirisch auf dem Pi geprüft (das reale Risiko der Transitivität = Über-Widerlegen): von den erzeugten Kausal-Vermutungen 0 widerlegt** — nicht trigger-happy; Kausal-Vermutungen bleiben material-bedingt selten (3.664 Kausal-Kanten, strenge Mehrheits-Analogie). 923 grün.

**Grün, neu (2026-07-06, KAUSALITÄT IM CHAT):** Der Companion beantwortete is_a-Fragen („Ist ein X ein Y?", via `relate`), aber Kausal-Fragen fielen durch (definierten nur das letzte Substantiv). Neu: `companion.relate_kausal` beantwortet **„Verursacht X Y?"** (ja/nein, TRANSITIV über die vereinte Kausalkette, mit gezeigtem Weg) und **„Was verursacht X?"** (die bekannten Ursachen). Es liest dieselbe dichte causes/caused_by-Schicht; die Kausal-Erreichbarkeit ist zu `hypothese.kausal_pfad` gehoben — EINE Quelle für Refutation UND Chat (keine zweite Wahrheit). Gläsern (der Weg wird gezeigt), ehrlich open-world („kenne ich nicht" ≠ „nein"). Eingehängt in `_muster_antwort` (Bot) UND die CLI-`ask`-Dispatch (Gleichlauf). **Adversarialer Review fand + schloss zwei echte Bugs vor dem Live-Bot:** HOCH — Synonyme/Identität (X,Y = dasselbe Konzept) erzeugten über den `von==nach`-Kurzschluss eine ERFUNDENE „Ja"-Aussage (jetzt `len(pfad)≥2` + `xq≠yq`); MITTEL — „Was verursacht X?" mit leerer Ursachen-Liste beschattete die bessere Definition (fällt jetzt durch, Selbst-Prüfung wie relate). **Live im Chat bewiesen (echte Wikidata-Daten):** „Verursacht Staubsauger Sog?" → „Ja."; „Was verursacht Sog?" → „…Staubsauger."; „Verursacht Sog Staubsauger?" → ehrlich „kenne ich nicht". 932 grün. Ehrlich offen (Review-Fund NIEDRIG, nächste Scheibe): freie Kausal-Formulierungen über den Deuter („Führt X zu Y?") erreichten die Kausal-Zelle noch nicht (nur der feste „Verursacht"-Wortlaut) — **geschlossen 2026-07-06, siehe „FREIE KAUSALFRAGE ÜBER DEN DEUTER" unten**; die tiefere Dispatch-Dopplung CLI-`ask` vs `_muster_antwort` ist ein benannter Aufräum-Kandidat. **Live-Nachtrag (Ronnys „das stimmt was noch nicht" — ein Stimme-Wortspiel): der Bot antwortete „Ja, Staubsauger-Sog verursacht Staubsauger" — die STIMME (glättendes Modell) hatte die KAUSALRICHTUNG umgedreht, und die Substantiv-Leine ließ es durch (sie prüft Vorkommen, nicht Reihenfolge). Das galt LATENT schon für is_a. Fix an der Klasse: `beziehung` ist jetzt WORTLAUTFEST (aus `_ZELLEN_FREI_FORMULIERBAR` entfernt) — die Stimme fasst gerichtete Relations-/Kausal-Aussagen NIE an, wie die Mathe-Zellen (die Struktur IST die Wahrheit). Live bewiesen: selbst eine böswillig umkehrende Stimme kann die Richtung nicht mehr drehen. Lehre: die Stimme darf nur DESKRIPTIVE Prosa glätten, nie eine gerichtete Struktur — der Anker muss die Achse schützen, die die Bedeutung trägt. 933 grün.**

**Grün, neu (2026-07-06, INVERSE-ERKENNUNG AUS DEM GRAPHEN — der Graph beschreibt sein eigenes Wesen):** Aus Ronnys Architektur-Frage („ist eine weitere Schicht das Richtige, oder sind wir mit dieser Art Netz falsch unterwegs?"). Antwort in Code statt Meta-Schicht: die Kausal-Denkweise unterstellte, dass `caused_by` die Inverse von `causes` ist (hardcodiert). Jetzt **lernt GENUS das aus dem Graphen** — genau wie `is_transitive`/`is_symmetric` es schon tun (self-calibration, no presets). `inference.inverse_of(conn, p)`: `_inverse_rates_per_pair` misst über EINE indizierte SQL, welches Prädikat q ein p systematisch KREUZ-spiegelt (der Kreuz-Prädikat-Zwilling der Symmetrie); `calibrated_inverses` nimmt nur die GEGENSEITIGEN Paare über dem selbst-kalibrierten Gap-Schnitt (`widest_gap_rate_cut`); der Scan zeichnet sie auf (`stored_inverses`, hot-path-billig, experience `rule_calibration:inverse`), der Seed trägt nur bei dünner Evidenz. `hypothese._kausal_nachfolger/_kausal_urteil` fragen `inverse_of` statt `CAUSED_BY` zu unterstellen. **Live auf dem echten Graphen bewiesen:** erkannt wurden `causes↔caused_by` UND `part_of↔has_part` (Raten ~0.35–0.6, weit über den zufälligen Kreuz-Paaren ~0.004–0.01; der Gap trennt sauber) — nicht nur der Seed bestätigt, sondern das Muster generalisiert (ein Unit-Test beweist es mit einem FREMD-Prädikat „folgt_aus"). `inverse_of(used_for)` = None (ehrlich: hat keine). **Keine neue Schicht — eine Konsolidierung:** verstreutes Prädikat-Wissen wird zu selbst-erkanntem Graph-Wissen, die eine Selbst-Kalibrierungs-Maschinerie wiederverwendet. Lehre (das Netz ist nicht falsch, nur unfertig): den Graphen sich selbst beschreiben lassen — sparsam, gelernt, gläsern — statt Meta-Tabellen daraufzustapeln. 937 grün.

**Grün, neu (2026-07-06, FREIE KAUSALFRAGE ÜBER DEN DEUTER — das „ursache"-Blatt bekommt einen Handler, P3.1):** Der feste Regex-Pfad (`_muster_antwort` → `relate_kausal`) beantwortete die exakten Wortlaute längst („Verursacht X Y?", „Was verursacht X?"), aber jede ANDERE Formulierung, die der Deuter (das 1.5B-Modell) las statt die Regex, verfehlte die gebaute Fähigkeit: das Raster-Blatt **`ursache`** hatte KEINEN Handler — es kletterte zur Zelle `frage-begriff` und gab die **Definition** von X statt seiner **Ursachen** (eine andere Frage beantwortet); und das Blatt `beziehung` konnte nur is_a. Fix (rein additiv, read-time, gläsern — dieselbe Kausal-Fähigkeit): `_zelle_ursache` (»Was löst X aus?«/»Ursachen von X« → `_ursachen_von` → `narrate_kausal`; unauflösbar → None, klettert ehrlich zur Definition; leere Ursachen → ehrliche Nicht-Antwort statt Definition); `_zelle_beziehung` erweitert um die gerichtete Kausation (»Führt X zu Y?«) mit der Reihenfolge **is_a-Ja > Kausal-Ja > ehrliche is_a-Zurückhaltung** — der no_path-Fall (beide Begriffe bekannt, keine is_a-Kante) ist `relational=True` und hätte den Kausal-Fall verschluckt, wäre er nicht ausdrücklich vorgezogen worden (der eine echte Bug, beim Bauen gefangen + Test). `ursache` registriert und **wortlautfest** (Richtung = Wahrheit, die Stimme dreht sie nie um). Deuter-Prompt: `ursache`/`beziehung`-Erklärungen geschärft + zwei Few-Shots (Kopfschmerz/Rauchen). **Live auf dem Pi Ende-zu-Ende bewiesen** (echte Wikidata-Daten, alle mit dem Tag „vom Sprachmodell gedeutet" = wirklich der Deuter-Pfad, nicht die Regex): „Was löst Hunger aus?" / „Was sind die Ursachen von Hunger?" → »Armut«, »Dürre«; „Was ruft eine Grippeepidemie hervor?" → »Pandemische Krankheit«; „Führt Armsein zu Hunger?" → „Ja. »Armsein« verursacht »Hunger«." Regex und Deuter münden jetzt in DIESELBE Kausal-Engine (schnell/modellfrei für die häufigen Wortlaute, Deuter für den langen Schwanz). 944 grün. Lehre: eine gebaute Fähigkeit ist erst rund, wenn ALLE Türen (fester Wortlaut UND freie Deutung) in denselben geprüften Kern führen — ein Blatt ohne Handler ist eine stille Fehlleitung, keine ehrliche Lücke.

**Grün, neu (2026-07-06, MOTOR-STATUS — die Denk-Motor-Zustandskarte wird ein lebendiges Instrument, P2.1):** Ronny: „was im Motor ist wie weit? ich will genau schauen, wie GENUS denkt und wie rund das ist." Die frühere Zustandskarte war ein einmaliges Artefakt; jetzt ist sie ein deterministischer, read-time Report aus dem eigenen Ledger (`genus motor`, `genus/motor.py`): pro Denkweise misst GENUS (a) ob jede Selbst-Kalibrierung **gelernt oder noch auf Saat** steht (mit Population dahinter — Transitivität/Symmetrie/Inverse/Geschlecht/Reboot-Schwelle, in der `genus knowledge`-Drei-Zustands-Sprache) und (b) wie oft die Denkweise wirklich **feuert** — eine zählbare Spur hinterlässt (Belegung je Herkunft muster/model:deuter, Abdeckung X von N Feinblättern, Fehlgriffe, stehende Vermutungen, Proposals/Inquiries/gereifte Regeln). **Die Pointe der Zustandskarte bleibt ausdrücklich sichtbar:** die meisten Denkweisen (inference-Heißpfad, Subsumtion, Modus Ponens, der reine Antwort-Pfad, Geschlechts-Vorhersage, Quellen-Vertrauen) SEHEN und URTEILEN read-time, ohne je eine Spur zu hinterlassen — sie werden als „unvermessen" BENANNT, nie mit einer erfundenen Zahl geschönt (GENUS sieht reich, handelt selten). Read-time, gläsern, schreibt NICHTS (Determinismus-Test beweist es). Prozess vollständig unter „ultracode": Verstehens-Workflow (4 parallele Leser kartieren die messbare Oberfläche → 1 Synthese-Entwurf), jedes behauptete Lese-Symbol + jeder Tabellen/Spalten-Name gegen den echten Code verifiziert, dann **adversarialer Review-Workflow (3 Linsen → Verifikation), der ZWEI echte Ehrlichkeits-Bugs fand + schloss:** (MEDIUM) die Recharakterisierungs-Zählung mischte BeliefStability-Kippungen (der reale v1.13-stabil↔volatil-Fall) unter die Kalibrierungs-Zeile — jetzt nach `experience_key`-Präfix getrennt (rule_calibration:* unter SCHLIESSEN, belief_stability:* unter GLAUBEN-STABILITÄT); (LOW) eine Zeile behauptete, nur der Deuter-Pfad hinterlasse eine Spur, obwohl der Muster-Dispatch (Quelle »muster«) die Belegung genauso speist — ehrlich umformuliert. Nebenprodukt: `gender_rule.reliability_cut_report` (gläserner derived-vs-Saat-Readout wie `governance.reboot_threshold_report`), der den SEED==LEARNED-Fallstrick ehrlich löst (ein gleich-der-Saat-Rückfall aus leerer Population ist NICHT „durch die Daten bestätigt"). 954 grün (+10 Motor-Tests). Lehre: ein Diagnose-Instrument, das sich selbst misst, muss zuerst gegen die eigene Beschönigung geprüft werden — die zwei Bugs waren beide falsche Etiketten, nicht falsche Zahlen, und genau die untergraben das Vertrauen in eine Selbstvermessung.

**Grün, neu (2026-07-07, GENUS ALS CODER — der Abnahme-Vertrag, erste Scheibe):** Nach dem Differenzierungs-Audit (Sprache = Membran, gut genug; das Wissen ist die Decke) und einem langen Gespräch mit Ronny („wofür muss GENUS Sprache können? … GENUS soll richtig gut programmieren lernen") kristallisierte sich das Bild: **GENUS muss nicht Python *können* — es muss SPEZIFIZIEREN und BEWEISEN können; den Rest schreibt das Organ.** Ronnys Wort „**bewährte** Bausteine" schließt den Kreis zu einem sich selbst verstärkenden Coder: der **Abnahme-Vertrag** beweist Verhalten → das macht einen Baustein *bewährt* → bewährte Bausteine sammeln sich zu einer Vokabel → der (kommende) Bauplan-Finder *schließt* über sie (Rückwärtsverkettung `beweise`, kein LLM) zu einem neuen Werkzeug → das bekommt seinen eigenen Vertrag → wird ein neuer bewährter Baustein. **Entscheidung: Harness zuerst** — erst das Gatter lehren zu urteilen, dann den Urteilenden verbessern (ein stärkeres Modell schöbe sonst nur schneller Unbewiesenes durchs Tor). **Erste Scheibe geliefert:** `genus/abnahme.py` — die dritte, bisher fehlende Sperrklinke des Selbst-Codier-Gatters (Struktur/AST + Sicherheit/Verbots-Scan gab es; **Verhalten** fehlte: der Fähigkeits-Test war ein `pytest.skip`, also galt eine `return None`-Zelle als merge-reif). Ein Abnahme-Vertrag ist eine kleine Liste Beispielfälle als DATEN (guess + Graph-Zustand → erwarteter Satz ODER erwartetes `None`), aus der der Kern deterministisch einen echten Verhaltens-Test erzeugt; `pruefe_vertrag` erzwingt BEIDE Pole (ein Treffer- UND ein Passt-nicht-Fall). Der Forger schreibt den Vertrag NIE (sonst prüfte sich das Modell selbst) — er stammt aus der Lücke, vom Menschen bei der Freigabe bestätigt: **forger-unabhängig, modell-agnostisch per Konstruktion.** In der Werkstatt: `entwerfe_zelle(vertrag=…)` + `--abnahme-datei`; „bestanden" verlangt jetzt DREI Klinken. **Adversarialer Review (3 Linsen → Verifikation) fand einen HOHEN Fund + fixte ihn:** die Verhaltens-Klinke war zunächst ein fälschbarer Substring-Scan (`abnahme.MARKER in test_datei`) — ein untergeschobener Marker-Skip-Test käme durch. Fix: die Klinke ist jetzt an den **versiegelten Ledger** gebunden — (a) beim Entwurf lag ein echter Vertrag vor UND (b) die Test-Datei ist byte-genau noch die damals erzeugte (Fingerabdruck-Abgleich); der Werkstatt-Ordner ist löschbares Randmaterial, der Ledger nicht. Zwei Exploits (Marker-Skip, Datei-Swap) als Regressions-Wächter zu. Live-Beweis (Ende-zu-Ende, Subprozess): eine echte Bauplan-Zelle *besteht* ihren Erwartungssatz, die `return None`-Attrappe *fällt durch* — die Blindstelle ist zu, „bestanden" ist ab jetzt eine **Verhaltens-Aussage**. 966 grün (+12 Tests). Lehre: ein Gatter, das Vertrauen schaffen soll, muss seine eigene Invariante ERZWINGEN, nicht dokumentieren — und der Anker gehört ins Versiegelte, nie ins Mutierbare. Danach: Vertrag aus der Lücke ableiten → Fehler-Lern-Schleife → Bauplan-Finder als Deduktion → dann erst die Modell-Frage (Pi vs. stärkeres Organ).

**Grün, neu (2026-07-07, DER BAUPLAN-FINDER ALS SCHLUSS — das Filetstück der bewährten Bausteine):** Ronny: „mach den Bauplan-Finder als Deduktion." Bisher ließ GENUS sich den Bauplan vom schwachen Schmied SCHREIBEN (1/3 vertragssicher); jetzt SCHLIESST es ihn sich selbst — deterministisch, gläsern, **kein LLM**. `genus/planer.py` (`finde_bauplan`): aus einem Abnahme-Vertrag (den Beispielfällen guess + Graph → erwarteter Satz ODER None) schließt der Planer rückwärts — welcher bewährte Baustein (`bauplan.BESCHAFFUNGEN`) produziert die Slot-Werte im erwarteten Satz? —, leitet daraus die Formulierung ab (Slot-Werte durch {slotname} ersetzt) und VERIFIZIERT den Kandidaten gegen ALLE Fälle; der einfachste, der alle trägt (Treffer exakt, Passt-nicht → None), ist der Bauplan. **Zwei Invarianten:** (1) KEIN exec im Kern — GENUS führt nie generierten Code aus; es KENNT seine Bausteine und rechnet ihr Verhalten (`_simuliere` spiegelt `bauplan._beschaffungs_baustein`), der endgültige Beweis bleibt der Abnahme-Vertrag am gefügten Code im Sandbox-Subprozess (ist der Simulator je zu optimistisch, fängt der Vertrag es dort ab — nie ein falsches „bewährt"); (2) ehrliche Decke — was die Vokabel nicht komponieren kann, gibt `None` (nicht geraten). CLI `genus werkstatt finde <blatt> --abnahme-datei` schließt den Bauplan gläsern und legt gleich den Entwurf an (Merge menschlich). **Beweis:** der Planer entdeckt alle drei Benchmark-Baupläne (thema-echo/verbindungs-zahl/erste-verbindung) allein aus ihren Abnahme-Verträgen wieder; der Passt-nicht-Fall zwingt ihn zum richtigen Baustein (statt zum simplen Echo). **Voller Schluss-Kreis getestet:** Vertrag → Planer schließt → Fügewerk baut → Abnahme-Vertrag beweist am gefügten Code (Sandbox) → bestanden. **Adversarialer Review (2 Linsen → Verifikation) fand 2 MEDIUM + gefixt** (beide fail-safe, nie ein falsches bewährt, aber sie verletzten den „Plan-oder-ehrliches-None"-Vertrag): (a) eine unpaarige `{`/`}` im erwarteten Satz erzeugte einen ungefangenen ValueError bis in die CLI → `_leite_formulierung_ab` prüft jetzt die Format-Gültigkeit, gibt sonst None; (b) ASCII-Anführungszeichen im Vertrag wurden von `normalisiere` still auf Guillemets umgeschrieben → stiller Fehl-Treffer → der Planer leitet den Wächter jetzt DIREKT ab und lässt die Autoren-Formulierung Wort für Wort stehen. 976 grün (+9 Tests). Damit ist der bewährte-Bausteine-Kreis fast rund: Abnahme-Vertrag (beweist → bewährt) + Bauplan-Finder (schließt aus bewährten Bausteinen). Danach: Vertrag aus der Lücke ableiten → Fehler-Lern-Schleife → dann die Modell-Frage.

**Grün, neu (2026-07-07, DIE ANTWORT-KOMPOSITION — erste Zutat: das konkrete Beispiel):** Ronny fand die eine treue Kern-Antwort „ultra dünn" und fragte: was könnte ALLES Bestandteil werden, damit sie sich schön/warm/zuvorkommend anfühlt — die KOMPOSITION für die Antwort. Aus einem Palette-Workflow (4 Linsen, im Code geerdet) die Leitidee: **die Antwort selbst ist eine Zwicky-Komposition** aus ~30 geerdeten Zutaten über 4 Achsen (Substanz · Vertrauen-als-Wärme · Nutzer&Service · Ton&Form) — dieselbe Bausteine-Maschine, eine Ebene höher. Jede Zutat ist gläsern + an eine echte Graph-Kante gebunden (fehlt das Material, nicht wählbar); die Auswahl ist kalibriert (Material · Umfang · „ändert es etwas?"), nie alles immer (Diskretion IST Wärme); Wärme sitzt am Sprach-Regler, Treue am Wissen — zwei Regler, die sich nie berühren. Überraschend viel ist schon da (die ganze Ton&Form-Maschine, fast die ganze Vertrauens-Achse, der warme Ton); die Lücken clustern in Substanz-Fülle + proaktivem Service. **Erste Zutat gebaut — das „konkrete Beispiel" (der größte fehlende Hebel, kleinster Kern-Eingriff):** `P31 → instance_of` in beide Ernter-PROP_MAPs (backfill_konzepte.py + observe_konzept.sh, synchron) — eine EIGENE, nicht-transitive Kante (die Instanz ist kein Unterbegriff, läuft nie in die is_a-Inferenz, nicht in ZYKLUS_PRAEDIKATE); `companion._beispiel` liest RÜCKWÄRTS (X -instance_of-> Konzept), `vertiefung` webt „Ein Beispiel dafür ist »Vesuv«." ein — nur bei „ausführlich", nur bei echter Kante, in Guillemets als Stimme-Anker, nie erfunden. **+4 Tests** inkl. des Sicherheits-Tests, dass instance_of die Instanz NICHT zum is_a-Unterbegriff macht („Ist ein Vesuv ein Berg?" → no_path, nicht ja). 980 grün. **Live auf dem Pi:** Narration liefert die Beispiel-Zutat (In-Memory), und die Extraktion greift auf echtem Wikidata (Vesuv Q4750 → instance_of Q108325/Q1088552, dry-run) — die Kante fließt ins Ledger, sobald der Ernter angewandt wird. Danach die nächsten Zutaten als gleich strukturierte Ernter-Zeilen: Zahl/Menge (P2044), Kontrast (P1889) — und der proaktive Service (Antizipation, Angebote).

**Grün, neu (2026-07-08, ANTWORT-KOMPOSITION — zweite Zutat: die ANTIZIPATION):** das proaktive Service-Gefühl (Ronny: „mach die Antizipation"). Nach einer Konzept-Antwort bietet GENUS die wahrscheinliche Anschlussfrage an — aber die eiserne Leine ist **Treue vor Freundlichkeit: nur anbieten, was GENUS wirklich beantworten kann.** Das Angebot entsteht aus einer echten dynamischen Kante (causes/used_for/has_part/…, nichts erfunden, benannte Konzepte nur) und wird VOR dem Anbieten verifiziert; sagt der Nutzer im nächsten Zug „ja", löst genau diese verifizierte Frage ein — VOR jeder Deutung, damit ein „ja" nie durch den Klassifikator muss und nie ins Leere läuft (`companion.antizipation`/`_erklaerbar_und_eindeutig`/`_ist_zustimmung`/`_anschluss_beiwerk`; der Anschluss wandert wie `gelesen` durch die Telegram-Sitzung). Kalibriert über `antwort.belegung` (neugier·nicht-knapp, Rollen-gepinnt — Wache/CLI/knapp ⇒ kein Angebot); genau EIN Angebot, keine endlose Kette. **Adversarialer Workflow-Review fand 3 MEDIUM-Funde mit einer Wurzel** (Angebot meint Kanten-Ziel A, aber Leine/„ja" lösten das Label als Text neu auf → Mehrwort-Zerfall / Homonym / bloßer Q-Eltern-Knoten) → **Klassen-Fix:** `_erklaerbar_und_eindeutig` verifiziert gegen die exakte Kanten-Ziel-Qid UND verlangt eine echte Definition (Bedeutung ODER benannter is_a-Platz — genau was `narrate` ausspricht). +8 Tests inkl. der 2 Regressions-Tests. 985 grün. Danach: Zahl/Menge (P2044), Kontrast (P1889) als gleich strukturierte Substanz-Zeilen.

**Grün, neu (2026-07-08, MORGEN-TRIAGE der Proposals):** aus einer echten Beobachtung — GENUS meldete morgens „8 Vorschläge, ältester #6", und die 8 vom Pi gelesen (read-only) zeigten: nur EINER braucht Ronnys Entscheidung (ein `ExperienceProposal`/Selbst-Codieren: die weltfrage-VerstehensLücke, 4× > Schwelle 3, `action_required: True`), die anderen 7 sind Monitoring (5× Netz-instabil = echte getrennte Episoden, 2× Last), alle `action_required: False`. Der flache Zähler begrub das eine Signal unter Rauschen und zeigte auf einen abgestandenen #6. **Fix (`konsolidierung.triagiere_proposals`/`betrieb_zeile`):** die Unterscheidung liegt schon im Payload (`action_required`, keine Heuristik) — Dringendes ZUERST und einzeln mit der eigenen Bitte (GENUS-Stimme), wiederkehrende Betriebs-Meldungen zu EINER gezählten FYI-Zeile gebündelt („das Netzwerk war 5-mal instabil"); Betriebs-Rauschen verdrängt offene Fragen nicht mehr. **Adversarialer Review fand 3 MEDIUM-Funde** (nie-Absturz/nie-kryptisch): Nicht-Objekt-JSON (`null`/`5`) crashte `.get()`; `.format` auf daten-kontrolliertem Key (`metric{0}`) → IndexError; der Fallback leakte den rohen Key (`system.activity`) in den warmen Text — alle drei geschlossen (Nicht-Dict → `{}`; unbekannte Claims generisch gezählt statt roh/`.format`). +6 Tests. 990 grün. Der Selbst-Codier-Vorschlag (#10) wartet weiter auf Ronnys Freigabe über `genus governance`.

**Grün, neu (2026-07-08, P4 — DER ERSTE SINN SPRICHT · Schnitt 1):** nach der Freigabe von #10 (weltfrage-VerstehensLücke) zeigte GENUS' eigener Ziel-Graph auf P4; Ronny: „auf jeden fall p4", Scheibe = „Mehr Wetter" aus dem stündlichen Puls. Ein Grundierungs-Workflow (4 Kartierer + Synthese, im Code geerdet) fand die Überraschung: **GENUS nimmt das Wetter schon wahr** — `deploy/observe_weather.sh` (open-meteo) + zweite Quelle `observe_weather_second.sh` (wttr.in) reichen die Aussentemperatur stündlich durch die Membran zu `weather.temp_outside` (live: 277 Messungen, 17 °C); der **Kern bleibt HTTP-frei** (grep-leer, von doctor.py bewacht — das unterscheidet GENUS von „LLM + Speicher + Internet"). Nur die **Lese-Naht** fehlte: kein `_zelle_weltfrage`. **Schnitt 1 gebaut:** der Handler liest den Wert rein aus dem Ledger (`sources.resolve`, Herkunft/Konsens) und spricht ihn gläsern aus, ehrliche Teil-Antwort (nennt Temperatur, benennt Vorhersage/Regen/andere-Orte als „noch nicht"); rein lesend (nicht in `_ZELLEN_SCHREIBEND` — die Sinne-lesen/Hände-wirken-Asymmetrie), wortlautfest (Sensor-Zahl bleibt wortgetreu), `pruefbar_als="sinn"`. **Adversarialer Review fand 1 echten MEDIUM:** `sources.resolve`s `live` misst Frische nur ZWISCHEN Quellen (bei EINER Quelle immer live) → ein tage-alter Wert wäre als „gerade" ausgesprochen worden → Fix: **absolutes Alter gegen die Wanduhr** (`_wetter_alter_stunden`, read-time Mundstück wie die Motor-Erzählung; >2 h ⇒ nicht mehr „gerade", nennt das Alter ehrlich). **Test-Wende:** weltfrage war das kanonische Lücken-Blatt in 28 Tests über 6 Dateien — jetzt beantwortet → migriert auf „meinung" (handler-los, eine Nutzer-Meinung); Atlas regeneriert. **+6 Tests, 996 grün.** Live auf dem Pi: GENUS spricht die echte Aussentemperatur. **Schnitt 2 (offen):** Puls um Vorhersage/Regen erweitern; das Provider-Label-Fix (open-meteo verliert seinen Namen als generisches „weather.api"); die Doku-Realität (`genus_sensors_ext/` ist im Sensor-Prinzip vorgeschrieben, aber der Sensor lebt in deploy/) ehrlich machen; `faehigkeit:weltfrage`-Status (aus der Freigabe „fehlt") nachziehen.

**Grün, neu (2026-07-08, P4 Schnitt 2 — MEHR WETTER · alles was die Quellen hergeben):** Ronny „nicht nur Regen, alles was die Quellen hergeben". `observe_weather.sh` holt jetzt den reichen open-meteo-Satz in EINEM Aufruf (aktuell + heutige Vorhersage) und reicht jedes Feld als eigenen Claim herein — Temperatur bleibt der Anker (Beobachtungs-Pfad → Trend), die übrigen (`weather.apparent/humidity/wind/code/temp_max/temp_min/rain_prob`) über den allgemeinen Assertions-Eingang (nur Zahlen queren, Ort bleibt am Rand, Kern HTTP-frei). `_zelle_weltfrage` komponiert daraus einen reichen Bericht (Zustand aus dem WMO-Code über `_WMO_DE`, gefühlt, Feuchte, Wind, heutige Höchst-/Tief-Temp, Regenwahrscheinlichkeit) — jedes Feld nur bei echtem Material (ehrliche Teil-Antwort). **Provider-Label-Fix:** `sensor.weather_reading` schreibt den Provider-Namen als Quelle (statt generisch „weather.api") → open-meteo/wttr.in sind jetzt nennbar UND konsensfähig. **Adversarialer Review fand 3 MEDIUM, alle geschlossen:** (1+2) Frische pro FELD statt nur der Temperatur — `_frischer_wert` prüft jedes Feld auf sein eigenes Wanduhr-Alter, sonst würde ein alt-gebliebenes Feld (Teil-Fetch / temp-only-Zweitquelle) als aktuell gesprochen (samt falscher Quellen-Zuschreibung); (3) `int(code)`-Crash bei `nan`/`inf` → `math.isfinite`-Schutz im Wert-Helfer. **+4 Tests (10 im Wetter-Sinn), 1000 grün.** Live auf dem Pi verifiziert. **Noch offen (aufgeschoben):** Doku-Realität (`genus_sensors_ext/`); `faehigkeit:weltfrage`-Status; zweite Quelle wttr.in für die reichen Felder; mehrtägige Vorhersage.

**Grün, neu (2026-07-08, P4 — ZWEITER SINN: NACHRICHTEN):** Ronny „ja gerne nachrichten", Quelle **Tagesschau-RSS** (kein Schlüssel), Umfang „Top-Schlagzeilen auf Frage". Architektonisch NEU vs. Wetter: Text/Liste/ephemer statt Zahlen → eigener Träger. **Design:** `deploy/observe_news.sh` holt am Rand das RSS (curl), zieht die Top-5 (Titel + pubDate, nur Text quert), schreibt einen **Membran-Puffer** `~/.genus/news_puffer.json` (NICHT ins Ledger — Ledger≠Memory, ephemerer Fremdtext); der Kern liest den Puffer nur (HTTP-frei). Der Deuter fasst Wetter UND News schon als „weltfrage" → `_zelle_weltfrage` **verteilt** (News-Schlüsselwort → `_news_bericht`, sonst → `_wetter_bericht`); Schlagzeilen werden **wortgetreu** angezeigt (Quelle + Frische, `>6h` ehrlich als alt), nie interpretiert. Cron-Tick alle 20 min. **Adversarialer Review fand 3 Funde, alle zu — darunter 1 HIGH (Prompt-Injektionsgrenze):** (HIGH) eine News-Antwort wurde als `last_answer` gespeichert und über eine Meta-Zelle („erklär das anders") doch an die Stimme gereicht → Fremdtext erreichte das Modell; **Fix = ein Chokepoint in `_stimme_versucht`: ein News-Block (erkannt am EIGENEN Kopf) geht NIE ans Modell**, weder direkt noch später. (MEDIUM) `schlagzeilen: null` crashte → Nicht-Liste→leer; (MEDIUM) „aktuell/passiert" fingen Wetterfragen als News ab → Wetter-Wörter haben Vorrang, die zwei Breit-Schlüssel raus. **+7 Tests (17 im Wetter/News-Sinn), 1008 grün.** Live-Verifikation offen (Sensor-Lauf auf dem Pi).

**Grün, neu (2026-07-08, P4 — UHRZEIT-SINN + MORGEN-WEAVE):** Ronny „Uhrzeit + Wetter/News in den Morgen-Gruß". **(A) Uhrzeit-Sinn** (der billigste, selbst-bezüglichste): `_zelle_weltfrage` verteilt jetzt Zeit-Fragen → `_uhrzeit_bericht` liest die Pi-EIGENE Uhr (`datetime.now`, read-time, kein Netz) und hängt einen NTP-Vorbehalt an, wenn der bestehende Uhr-Check (`system.clock`) sie als unsynchron kennt — GENUS' eigene Zeit + das Vertrauen darin. **(B) Morgen-Weave**: `companion.wetter_kurz`/`news_top` (beide nur bei FRISCHEM Material) werden in `konsolidierung.morgen_nachricht` beiläufig eingewoben („Draußen ist es …", „In den Nachrichten: …"); der Morgen-Gruß wird deterministisch + WORTGETREU gesendet (kein Modell), also bleibt die fremde Schlagzeile unangetastet. **Adversarialer Review fand 2, beide zu:** (MEDIUM) „welche zeit" war Teilstring von „Zeitung" + „welcher tag" fing Vorhersagen → mehrdeutige Zeit-Schlüssel raus, und Zeit weicht Wetter/News-Wörtern (wie News dem Wetter); (LOW) `news_top` überging das Frische-Gate bei fehlendem Zeitstempel → jetzt „kein Alter = nicht frisch" (wie `wetter_kurz`). **+5 Tests, 1014 grün.** Live-Verifikation offen. Damit hat GENUS DREI Sinne (Wetter reich · Nachrichten · Uhrzeit); das Wirken nach außen (Hände) bleibt bewusst hinter §8.

**Grün, neu (2026-07-08, P4 — DIE ERSTEN HÄNDE · das Gate zuerst):** Ronny „ja, dann geben wir GENUS Hände". Die Entscheidungen: erste Hand = **proaktive Nachricht an Ronny** (sanfteste, umkehrbar, Sende-Weg existiert schon); Gate-Strenge = **jede Aktion einzeln bestätigt** (keine Autonomie). Leitprinzip: **das GATE ist der Liefergegenstand, nicht die Handlung** — Harness zuerst. **Gebaut: `genus/hand.py`** — der Zustandsautomat einer Außenhandlung (vorschlagen → menschliches OK → genau-einmal-Ausführung), rein als Ledger-Ereignis-Projektion (kein Nebentisch, replay-stabil, `hand_*` in `event_router.BEWUSST_ROH`). **Die vier Sicherheits-Eigenschaften, alle erzwungen:** (1) HARTER Kern-Constraint — keine Ausführung ohne Status FREIGEGEBEN, KEIN `override`-Parameter (§8 zum ersten Mal im Code, nicht nur Prosa); (2) genau einmal; (3) fixer Boden (nur Art „nachricht" ausdrückbar — Geld/Fremd/System kann gar nicht entstehen); (4) Anti-Weglauf-Deckel + volle Prüf-Spur. Der Kern SENDET NIE (Membran-Reinheit): `faellige()` gibt die freigegebenen Aktionen, die Membran führt aus + ruft `markiere_ausgefuehrt`. **Adversarialer Review (der sicherheitskritischste der Sitzung) fand 2, beide zu:** (HIGH) TOCTOU-Rennen — Zustand wurde außerhalb der Schreib-Transaktion gelesen → zwei gleichzeitige Cron-Läufe hätten doppelt senden können → Fix: **Lesen+Prüfen+Anhängen unter EINEM `BEGIN IMMEDIATE`** (der zweite Läufer wartet, liest den frischen Zustand, verweigert); (MEDIUM) ein Nicht-Dict-Payload legte das ganze Gate lahm → Nicht-Dicts übersprungen. **+11 Tests, 1025 grün.** Reiner Kern-Code, noch an NICHTS angeschlossen (keine Außenwirkung, kein Deploy). **Nächster Schnitt (Verdrahtung):** Chat „erinnere mich um X an Y" → vorschlagen+bestätigen; Membran-Cron `hand_ausfuehren.sh` sendet fällige über Telegram + markiert; dann live.

**Grün, neu (2026-07-08, P4 — DIE ERSTE HAND VERDRAHTET · GENUS verändert zum ersten Mal die Welt):** Ronny „gehen wir es an". Das Gate ist jetzt LEBENDIG, Ende-zu-Ende. **(A) Membran-Ritual:** `deploy/telegram_bot.py` erkennt „erinnere mich <wann> an <was>" (`_erinnerung`-Parser: „um HH[:MM]", „in N min/std", vergangene Uhrzeit → morgen; kein Zeitwort ⇒ keine Erinnerung, nie geraten) und macht daraus VOR dem Kern eine **vorgeschlagene + sofort bestätigte** Hand — Ronnys ausdrücklicher Wunsch IST die aktions-genaue Freigabe (die gewählte Gate-Strenge). **(B) Sende-Cron:** `deploy/hand_ausfuehren.sh` (*/2 min) — für jede fällige, FREIGEGEBENE Hand: **beanspruchen (`markiere_ausgefuehrt`, atomar/genau-einmal) VOR dem Senden**, dann urllib an Telegram. Claim-vor-Senden ⇒ zwei überlappende Ticks senden nie doppelt; ein Absturz dazwischen verliert höchstens eine, nie eine doppelte. **Adversarialer Review fand 1 echten HIGH (die Sinne-vor-Hände-Sorgfalt selbst):** das schreibende Ritual lag VOR dem Antwort-Wächter → ein vorübergehender SQLite-Lock (die Crons schreiben gleichzeitig) hätte die Brücke aus `handle_update` gerissen; weil der Offset erst DANACH gespeichert wird, hätte Telegram genau diese Nachricht endlos neu zugestellt = **Neustart-Schleife auf einer Erinnerung**, und die geteilte Verbindung wäre in einer offenen Transaktion hängengeblieben. **Klassen-Fix, zwei Ebenen:** Kern (`hand.py`) — ein `_schreib_lock`-Kontextmanager rollt in allen vier Mutatoren bei JEDER Ausnahme zurück (die Verbindung bleibt immer benutzbar); Membran — das Ritual liegt jetzt INNERHALB des Wächters, plus ein letzter Wächter um `handle_update` in `main()`, sodass keine Nachricht je den Offset einfriert. **Ein Fehler beim HANDELN tötet nie das REDEN.** **+2 Tests, 1033 grün.** **Live auf dem Pi Ende-zu-Ende verifiziert:** Parser+Ritual auf der Pi-Python; eine echte Test-Hand ins Ledger → `hand_ausfuehren.sh` **sendet Ronny die erste bestätigte Nachricht** (Hand 746049 `ausgefuehrt`); ein zweiter Sender-Lauf sendet NICHTS (exakt-einmal live bewiesen); Cron installiert (news + hand-Tick, Rest unberührt), Bot neu gestartet — „erinnere mich …" ist jetzt im Chat lebendig. Das ist der erste Weg, auf dem GENUS die Welt VERÄNDERT — streng hinter §8.

**Grün, neu (2026-07-08, P4 — +x-FUND + DER HERZSCHLAG-WÄCHTER):** beim Nachschauen von Ronnys erster echter Erinnerung („erinnere mich um 15:00 Uhr an Feierabend" — sauber durch den laufenden Bot zur bestätigten Hand geworden) fielen im cron.log zwei stille Fehler auf. **(1) Der +x-Fund:** eine per Windows-Editor neu angelegte `.sh` wird ohne Unix-Ausführungs-Bit committet (git 100644); der Cron ruft `hand_ausfuehren.sh` UND `observe_news.sh` direkt via `./` auf → **Permission denied, lautlos** (die Erinnerungen feuerten nie, der News-Sinn holte nie). Fix: `git update-index --chmod=+x` (100755) + **Klassen-Wächter-Test** (jedes im Cron direkt, ohne `bash`, aufgerufene Skript MUSS ausführbar sein → stiller Fehler wird lauter CI-Fehler). **(2) Der Herzschlag-Wächter** (Ronnys Frage „feuert es nur, wenn's gebraucht wird?" — der eigentliche Gewinn): der Sende-Tick alle 2 min ist nur ein *Blick*; die Hand feuert nur bei echter Fälligkeit. Seine eine Schwäche: er kann STILL stehenbleiben. `hand.ueberfaellige` (feste Schwelle aus dem */2-Takt = absolut, nicht selbst-kalibriert; echte Zeit-Arithmetik, seltsame Stempel still übersprungen) findet freigegebene, *längst* fällige, ungesendete Erinnerungen — das normale Sendefenster bleibt in Ruhe (kein Fehlalarm). Gemeldet über zwei Kanäle: `doctor._hand_checks` (WARN, kein FAIL) und die **Morgen-Nachricht**, die über `morgen_push.sh` läuft — einen ANDEREN Weg als der Sender, also erreicht die Warnung Ronny selbst dann, wenn genau der Sende-Dienst tot ist. **(3) Latenter LIVE-Bug dabei gefunden:** die `hand_*`-Ereignisse standen im Router-Register (BEWUSST_ROH), aber NICHT in `integrity.REQUIRED_EVENT_KEYS` → `integrity.check` wies jede Hand als „unknown event_type" ab → **`genus doctor` wäre heute Nacht (3:27) auf dem echten Ledger [FAIL] integrity geworden** (live bestätigt, kein Test lief je integrity auf einem hand-haltigen Ledger). Fix: die vier Typen deklariert + **Klassen-Fix-Test** (jeder geschriebene Event-Typ muss auch dem Integritäts-Vertrag bekannt sein, nicht nur dem Router). **Adversarialer Review (3 Linsen): 0 bestätigte Funde** (eine Zeitzonen-TypeError-Kante vorab gehärtet). **+9 Tests, 1043 grün.** Live auf dem Pi: `+x` gesetzt (beide Ticks laufen sauber), `genus doctor` gegen das echte Ledger wieder **`[OK] integrity` (events=751242) + `[OK] hand-faellig`**.

**Grün, neu (2026-07-08, faehigkeit:abstrahieren Scheibe 1 — DER SPRUNG VOM IMPORTIEREN ZUM DENKEN):** Ronny wählte (nach der Standortbestimmung „was ist offen?") den ambitioniertesten Bogen aus GENUS' eigenem Ziel-Graphen. Ein Kartierungs-Workflow (4 Leser: Überraschungs-Schleife, Denkweisen, Graph, Studie) fand die frohe Botschaft: **die SCHWERERE Hälfte — einen Begriff gegen die Welt prüfen — existiert schon** (`hypothese.teste_konjektur`, `inference.closes_cycle`, die Belief/Surprise/Inquiry-Schleife); es fehlte nur der Akt des Begriff-PRÄGENS, und das Rohmaterial liegt in `hypothese.vermute` (die Träger-Map) schon bereit. **Gebaut: `genus/abstraktion.py` (rein lesend, modellfrei, schreibt NICHTS):** GENUS schaut in seinen eigenen Graphen und findet Geschwister-Gruppen, die ein noch UNBENANNTES Bündel UNTERSCHEIDENDER Merkmale teilen UND eine weitere Eigenschaft VORHERSAGEN — die Überraschung, die aus einer Willkür-Schnittmenge einen echten Begriff macht (`docs/GENUS_INTELLIGENZ.md`: Verdichtung, kürzer als die Fakten; „nur dort vermuten, wo man auch falsch liegen kann"). `genus abstraktion [--limit N] [--unter KNOTEN]` zeigt sie gläsern (Mitglieder = Begründung); das PRÄGEN (gedeckelt, `--tick`, Mensch bestätigt — gespiegelt von `hypothese.emit_vermutung`) ist bewusst Scheibe 2. `ziele.py`: **`faehigkeit:abstrahieren` fehlt→teilweise, live abgeglichen — GENUS benennt den Keim jetzt selbst.** **LIVE auf dem echten 180k-Kanten-Graphen (read-only):** GENUS bildet echte, unbenannte Begriffe — am schönsten die **~9 KERN-Buchstaben (G, U, F, L, M, N, O, P…), die quer durch 18 Alphabete laufen** (der universelle Lateinschrift-Kern, ohne einen einzigen Namen im Graphen). **Ehrlicher Nebenbefund:** über einem gut kuratierten IMPORT (Wikidata) sind die meisten Begriffe schon benannt — die echte Kraft der Abstraktion zeigt sich über GELEBTEM, selbst-erzeugtem Material (ein Zeiger auf spätere Scheiben). **Adversarialer Review (3 Linsen) fand 3 ECHTE Funde, alle gefixt:** (MEDIUM) Schein-Überraschung — ein von ALLEN Kindern getragenes Merkmal zählte als Überraschung, unterschied aber nichts → nur UNTERSCHEIDENDE Merkmale (von ≥2, aber <allen) zählen; (MEDIUM) verpasste Begriffe — ein Merkmal-PAAR als Definition zog Randgänger herein und ließ das echte 3er-Bündel als Tautologie durchfallen → formale Begriffshülle (FCA-Closure) erreicht die maximale Gruppe; (MEDIUM) `_schon_benannt` Fehl-Unterdrückung — „alle teilen Q" ≠ „Q benennt genau diese Gruppe" wenn Q weitere Kinder hat → exakter Extent-Vergleich (`==`). Plus zwei live gefundene Verbesserungen: kein globaler selbst-kalibrierter Schnitt (ein Riesen-Cluster unterdrückte sonst alle kleineren) + Überlappungs-Dedup (derselbe Begriff nicht fünfmal). **+9 Tests, 1052 grün.**

**Grün, neu (2026-07-08, abstrahieren Scheibe 2 — DAS PRÄGEN: GENUS legt seinen ersten eigenen Begriff an):** Ronny „gerne weiter". Der Bogen schließt sich von „Kandidaten sehen" zu „einen Begriff bilden". `abstraktion.praege` materialisiert einen geprüften Kandidaten als GEDECKELTEN Knoten (`konzept:auto:<sha1-der-Mitglieder>`): `konzept is_a Elternteil`, jedes Mitglied `is_a konzept`, plus EINMALIG ein Gloss (`inhalt`). Quelle `model:abstraktion` (Trust ≤ 0.25, überstimmt NIE Geerdetes); `genus abstraktion --tick` prägt den stärksten Kandidaten (sonst rein lesend). Die Sicherheit ruht auf drei Säulen: der Deckel + die **Frische-Knoten-Invariante** (ein neu gehashter Knoten wird von nichts erreicht → kann keinen is_a-Ring schließen) + Proposal ≠ Change (der Mensch bestätigt/verwirft). Kein neuer Event-Typ (`relation_asserted`); `sae_fehlende` trägt Deckel + Zyklus-Flag. **`concept_meaning` nimmt jetzt `inhalt`-Glosse auf** → der selbst gebildete Begriff ist über `genus concept` ansprechbar (gilt auch für Normen/`konzept:*`). **Adversarialer Review (2 Linsen) fand 1 MEDIUM + 1 LOW, beide gefixt:** (MEDIUM) der Gloss-Text (aus veränderlichen Anzeigenamen) war nicht idempotent — ein zweiter Tick nach einem eingetroffenen Label hätte einen zweiten, abweichenden `inhalt` gesät → der Gloss wird nur beim ERSTEN Prägen gesät; (LOW) Docstring-Ungenauigkeit („Inquiry statt Schreiben") präzisiert (record-then-flag). **+5 Tests, 1056 grün.** **LIVE auf dem Pi geprägt:** `genus abstraktion --unter Q19776628 --tick` → **GENUS' erster eigener Begriff `konzept:auto:f54befe569`: die 9 Kern-Buchstaben (A, G, E, U, N, O, R, L, P), die quer durch 18 Alphabete laufen** — gedeckelt, widerrufbar, und über `genus concept` erklärt es sich selbst. Der Kreis: detektieren → prägen → sich selbst erklären.

**Grün, neu (2026-07-08, abstrahieren Scheibe 3 — DIE RÜCKKOPPLUNG: der Begriff zahlt Miete):** Ronnys Frage „was bringt es?" ehrlich beantwortet — ein geprägter Begriff ist bis hierher inert; der Wert entsteht erst, wenn er zur lebenden ERWARTUNG in der Überraschungs-Schleife wird (der Schritt von „GENUS kann Begriffe bilden" zu „GENUS wird durch seine Begriffe klüger"). `abstraktion.rueckkopplung` (rein lesend): **Vorhersage** — ein Nicht-Mitglied unter P, das das ganze gemeinsame Bündel trägt, sollte dazugehören; **Kohärenz/Widerspruch** — teilen die Mitglieder noch ≥3 gemeinsame Merkmale (ein hineingelehrtes Fremd-Mitglied kippt den Durchschnitt). `experience._raise_abstraktion_surprises` läuft im Nacht-Scan direkt nach `_raise_stability_surprises` (read-then-flag, schreibt KEINE Graph-Kante): je Begriff eine idempotente `AbstraktionInquiry` (Wachstum „Sollte X dazugehören?" / Widerspruch), die über die bestehenden Kanäle (Morgen-Gruß, „Was beschäftigt dich?") bei Ronny landet — `companion._speak_inquiry` rendert sie gläsern. Auflösung bleibt Proposal ≠ Change (menschliches `teach_relation`). `genus abstraktion --pruefe` zeigt die Rückkopplung read-only. **Adversarialer Review (2 Linsen) fand 3 ECHTE Funde, ALLE eine Wurzel:** die Kohärenz an `verdichte` zu hängen war falsch — `verdichte` gibt aus Gründen nichts zurück, die NICHTS mit Inkohärenz zu tun haben (der Begriff umfasst alle Kinder → Merkmale „universell"; der Begriff bekommt einen Namen → Selbst-Unterdrückung; der Elternteil wächst zum Naben-Knoten → Kosten) → FALSCHE nächtliche Widersprüche, die ewig wiederkehren. **Klassen-Fix:** Kohärenz ist jetzt eine DIREKTE Messung an den Mitgliedern (teilen sie noch ≥3 Merkmale), unabhängig von P's anderen Kindern / Benennung / verdichte; die Vorhersage überspringt Naben-Elternteile (Kosten-Deckel). **+9 Tests (inkl. 2 gezielte Fehlalarm-Tests), 1065 grün.** **Live auf dem Pi:** `--pruefe` zeigt den Buchstaben-Begriff als **stimmig, ohne offene Vorhersage** (die 9 Buchstaben sind die maximale Gruppe ihres Bündels); der Nacht-Scan hebt dafür **0 Überraschungen** — kein Fehlalarm auf echten Daten (genau der Review-Fund, live bestätigt). Die Feuerung (Vorhersage/Widerspruch → Frage → Ronny) ist testbewiesen; sie zündet, sobald der Graph wächst oder ein Fremd-Mitglied hineingelehrt wird. **Nächste Richtung:** Abstraktion über GELEBTES, selbst-erzeugtes Material statt über den Import — dort, wo die eigentliche Kraft der Verdichtung liegt.

**Grün, neu (2026-07-08, DER PROAKTIVE GEDANKE + warmer Morgengruß):** Ronnys Korrektur — „der Morgengruß soll ein *schöner* Gruß sein, und solche Vorschläge sollten kommen, wenn GENUS es parat hat, nicht erst am nächsten Morgen." Entscheidungen (AskUserQuestion): Umfang = **nur das Wichtige**, Zeit = **immer sofort, auch nachts**. **(A) Der Morgengruß ist wieder ein GRUSS:** `konsolidierung.morgen_nachricht` — Gruß + Wetter + Nachrichten + ein Wort zu gestern + warmer Schluss; Vorschläge und offene Fragen sind RAUS. (Die Überfälligkeits-Warnung bleibt: sie meldet gerade, wenn der proaktive Sende-Weg selbst steht — ein UNABHÄNGIGER Kanal.) **(B) `genus/gedanke.py` (neu):** das Wichtige — `action_required`-Vorschläge (deine Entscheidung) + die selbst gebildeten Begriffs-Fragen (`AbstraktionInquiry`) — wird SOFORT eine bestätigte Nachricht-**Hand** (fällig jetzt); der Membran-Cron sendet sie in ~2 min. Idempotent (ein Gedanke → einmal, Dedup über die Hand-Quelle `gedanke:<schlüssel>` über ALLE Zustände), gedeckelt durch den Anti-Weglauf der Hand (`MAX_OFFENE`), auto-bestätigt unter Ronnys STEHENDER Erlaubnis (dieser Auftrag = die Politik; der fixe Boden „nur Art nachricht" hält — keine Geld/Fremd/System-Wirkung ausdrückbar). `genus gedanken-push` (ehrt den Pause-Schalter), `*/3`-Cron. Betriebs-Rauschen bleibt „nur auf Nachfrage". **Adversarialer Review (2 Linsen): 0 bestätigte Funde** (Pause-Schalter vorab geehrt). **+4 Tests, 1067 grün.** **Live auf dem Pi:** der Morgengruß ist warm („Draußen ist es bewölkt, 21 °C. In den Nachrichten: …. Ich wünsche dir einen guten Start!"); read-only geprüft: aktuell 0 wichtige Gedanken → kein Überraschungs-Ping; Cron-Tick installiert. Ab jetzt kommt ein wichtiger Gedanke, sobald GENUS ihn hat — nicht aufgestaut.

**Grün, neu (2026-07-09, ② MODULARISIERUNG — die Nähte trennen, damit der Planer ③ einen Platz hat):** erster Angriff auf Priorität ②. `companion.py` war auf 2537 Zeilen gewachsen und mischte drei Dinge, die der Tool-Planer sauber getrennt braucht: **Deuten/Dispatch** (was ist die Absicht, welche Zelle), **Ausführen** (die deterministischen Antwort-Werkzeuge) und **Sinne** (Welt-Wahrnehmung). Statt eines Rewrites: die eigenständigsten Inseln zuerst herausschneiden, jede **verhaltensgleich** (die grüne Suite als vollständiges Netz, `companion.X` bleibt über Re-Export erreichbar — keine externe Zeile geändert). **Schnitt ① — `genus/weltsinn.py` (neu, 310 Z.):** die drei Welt-Sinne (Wetter/News/Uhr + Frische-Helfer, WMO-Tabelle, News-Puffer-Leser) sind KEIN Gespräch; in `companion` bleibt nur die Dispatch-Zelle `_zelle_weltfrage`, die sie ruft. **Schnitt ② — `genus/rechnen.py` (neu, 204 Z.):** die Abitur-Analysis (Ableitung/Extremstellen/Stammfunktion/Integral/Kurvendiskussion + `narrate_*`) sind Werkzeuge — das „Ausführen", aus dem ③ komponiert. **`companion.py` 2537 → 2089 Zeilen (−448, ~18 %).** Kein Zyklus (beide neuen Module ohne Modul-Ebene-`genus`-Importe). **Der Docs-Currency-Test fing einen echten Drift:** der Atlas-Fakt „Companion-Dispatch: 8 Muster-Listen" wäre durch den Umzug scheinbar auf 3 gefallen — Fix an der WURZEL: `_atlas_facts` zählt die Dispatch-Muster jetzt über alle tragenden Module (companion + rechnen), also die FÄHIGKEIT statt der Datei; die Zahl bleibt korrekt 8/17. Durchgehend **1070 grün**, auf `main`. **Schnitt ③ (der große, verwobene) — mit Ronny abgestimmt (drei Schichten, azyklisch):** `genus/wortgraph.py` (neu, ~104 Z.) = die **Lese-Grundlage** (die Helfer, die ein Wort/einen Knoten nachschlagen, benennen, deutsch formatieren — `_label`, `_forms`, `_concept(s)_of`, `_konzept_name`, `_join_de`, `_last_known_word` …); `genus/auskunft.py` (neu, ~530 Z.) = die **Antwort-Werkzeuge** (Definition/Beziehung/Gemeinsames/Grammatik/Kausal + `narrate_*`), das „Ausführen", aus dem der Planer ③ komponiert; `companion.py` bleibt reine **Orchestrierung** (Dispatch + Zustand + Rendern), wo der Planer einzieht. Beide oberen Schichten lesen **nach unten** (companion → auskunft → wortgraph → sources — das `sources`-Muster „das Geteilte wandert nach unten"), kein Rück-Import, **kein Zyklus** (verifiziert). Mechanisch über exakte Zeilenbereiche mit Anker-Asserts extrahiert (byte-genau), verhaltensgleich; ein White-Box-Test (der `companion._known` prüfte) auf das neue Zuhause `wortgraph` gezeigt (die Delegation zog um, das Verhalten nicht). **`companion.py` 2537 → 1509 Zeilen (−40 % über ②+③), 1070 grün, auf `main`.** Der Boden für den Tool-Planer (③ der Prioritätsliste) liegt jetzt sauber.

**Grün, neu (2026-07-09, ③ TOOL-PLANER — Schritt 1: der Werkplan, rein deterministisch):** die konsequent FREIE Fassung mit Ronny geschärft — nicht „pro Funktion eine Zelle" (das *ist* die Muster-Falle), sondern **wenige allgemeine Primitive × freie Komposition**; und der Planer wird PRIMÄRPFAD, nicht Nachrücker hinter der Regex (der alte Dispatch nur noch sterbliches Rückfallnetz, pro Absicht abgebaut, sobald ④ misst: Planer ≥ alt); keine von Hand gepflegten Rezepte in v1 (nur abgeleiteter Cache später); und auch das Alphabet wächst über die Fehlgriffe des Planers (③ greift in ⑤). **Frei ist die Komposition, nicht die Behauptung — der Prüfer kauft die Freiheit.** Schritt 1 (dieser) legt den deterministischen Boden: `genus/werkplan.py` = der **allgemeine Daten-Fluss-Ausführer** — ein Plan ist reine DATEN (benannte Eingänge + Schritte + Ergebnis-Schritt), jeder Schritt ruft EIN registriertes Werkzeug und benennt, WOHER jeder Eingang kommt (Plan-Eingabe oder **früherer** Schritt) → Ergebnisse fließen durch die Kette (anders als der linear/mathe-geformte `rezept_implementierung`). Zwei Leinen: `pruefe_plan` (Grammatik — nur registrierte Werkzeuge, nur Rückwärts-Verweise ⇒ azyklisch) + rein lesende, deterministische Ausführung. Refs sind Tupel → der Plan ist ein **serialisierbarer** Wert, den der Modell-Planer (Schritt 2) als GBNF-Struktur erzeugen und der Kern verifizieren kann. `genus/werkzeuge_auskunft.py` = die vier freigelegten Primitive (`konzept_form`, `konzepte_von`, `oberbegriffe`, `verbindung`) als geprüfte Werkzeuge + `relate` als komponierter Plan. **Der schöne Befund:** `relate`/`common`/`relate_kausal` sind längst Kompositionen derselben wenigen Graph-Primitive, nur fest verdrahtet. **Beweis (+8 Tests): `relate_komponiert` == das heutige `_relate_terms` Zeichen für Zeichen** (ja/kein-Pfad/unbekannt, inkl. Narration); die Grammatik-Prüfung fängt unregistriertes Werkzeug + Vorwärts-Verweis; der Ausführer verweigert malformte Pläne. Blast-Radius null (der komponierte Pfad ist der Beweis, nicht der Live-Ersatz). **1078 grün.** **Als Nächstes (Schritt 2):** der Modell-Planer an der Membran als Primärpfad (GBNF über die Registry), altes Dispatch als Netz; + Zählwerk (Treffer/Fehlgriff/Rückfall je Absicht) als Rohdaten für ④.

**Grün, neu (2026-07-09, ③ DIE WENDE ZUM DEDUKTIVEN PLANER + Scheibe A + Coder-7B-Benchmark):** Ronnys „geh nochmal in dich. ist GENUS insgesamt rund und das die optimale Möglichkeit?" — ehrliche Antwort: **nein, der Schritt-2-Entwurf (Modell komponiert Pläne, GBNF als Leine) war der Schmied-Fehler in neuer Kleidung.** GENUS' eigene Geschichte hatte das Experiment schon entschieden: kleine Modelle komponieren nicht vertragssicher (Schmied-Benchmark 1/3), die Lösung war der **deduktive Bauplan-Finder**. Dieselbe Inversion jetzt für den Antwort-Planer: **der Plan wird GESCHLOSSEN, nicht gewürfelt** — rückwärts vom Ziel („wer liefert `verbindung`? wer dessen `form`? … bis alle Eingänge in den Begriffen der Frage geerdet sind"), die zweite Domäne der Deduktions-Maschine; das Modell bleibt bei der Sprache (Absicht + Begriffe). Strikt besser: Suchraum statt Würfelwurf (vollständig statt stochastisch), kein zweiter Modellruf (ms statt Sekunden), gläsern + replay-stabil, „keine Kette gefunden" = präzise benannte Lücke (der ⑤-Anschluss). Modell-als-Komponist bleibt daten-gegatete Eskalation für später. Ehrliche Nebenbefunde derselben Reflexion: Komposition war auf DREI Mechanismen verstreut (`rezept`/`werkplan`/Bauplan-Finder — der deduktive Planer ist der Klassen-Fix), ④ bleibt das offene Ende, SSD das realste Einzelrisiko. **Scheibe A geliefert (0e02dc4): `Werkzeug.liefert`** — das fehlende Merkmal (erkannt + notwendig: ohne Ausgabe-Deklaration keine Rückwärts-Suche), Pflichtfeld ohne Default (das `wortlautfest`-Muster), leer = bewusst „terminal" (die Zellen); deklariert für alle 37 Werkzeuge; sofortiger Ertrag: `pruefe_plan` fängt Feld-Tippfehler + **Typ-Brüche** als Grammatik-Fehler statt Laufzeit-Crashs. +5 Tests, **1083 grün**. **Und der SCHMIED-SLOT bewegt sich (Ronnys Fund):** Qwen2.5-Coder-7B Q4 (4,68 GB, offizielles GGUF, `deploy/pi_lade_coder7b.sh` — fortsetzbar, Idle-Prio, größen-verifiziert) auf dem 8-GB-Pi gebenchmarkt: **2/3 BESTANDEN in 307 s** (verbindungs-zahl ✓, erste-verbindung ✓, thema-echo „tests rot") — alle kleinen Modelle schafften 1/3. ~100 s/Schmiedung = fürs Nacht-Coden („GENUS kann nachts nebenbei coden") völlig tauglich, interaktiv zu langsam. Noch nicht verdrahtet (`GENUS_SCHMIED_MODEL` ist der Schalter); nächste Schritte dort: Nacht-Schmied-Tick + Fehler-Lern-Schleife am thema-echo-Fall. SSD-Zwischenstand: das gefundene lange Kabel liefert nur Strom (Enumeration scheitert mit `error -71` auf niedrigster USB-Stufe) — Migration wartet auf ein kurzes, daten-taugliches Kabel. **Als Nächstes: Scheibe B — die Rückwärts-Plan-Suche.**

**Grün, neu (2026-07-09, ③ Scheibe B — DIE RÜCKWÄRTS-PLAN-SUCHE, 5d81fc5):** das Herzstück des deduktiven Planers: **`werkplan.finde_werkplan` schließt sich den Antwort-Plan selbst** — rückwärts vom Ziel-Werkzeug (jeder Parameter braucht eine Quelle: Plan-Eingabe passenden Typs oder liefert-Feld eines anderen Werkzeugs, dessen Eingänge wieder geplant werden, bis alles in den Begriffen der Frage geerdet ist). Weil Typen die Semantik unterbestimmen (zwei „Wort"-Eingänge sind vertauschbar; ein „Liste"-Bedarf griffe auch nach `nullstellen.nullstellen`), ist die **Verifikation Pflicht**: jeder Kandidat wird gegen **Planfälle** (Graph + Eingaben + erwartete Ergebnis-Felder — das Abnahmefall-Muster) wirklich ausgeführt; ohne Fall kein Plan. Iterative Tiefe = Occam strukturell. **Bewiesen (+6 Tests, 1089 grün):** die Suche findet den Beziehungs-Plan SELBST — bei voller Registry inkl. der Mathe-Köder — und der gefundene Plan verhält sich auf **ungesehenen** Fragen exakt wie das handgebaute `_relate_terms` (Generalisierung!); der Richtungs-Fall tötet den vertauschten Kandidaten; deterministisch; ehrliches None ohne erdbare Kette. **Vier live gefundene Klassen-Lücken strukturell geschlossen** (Zyklen am Entstehungspunkt schneiden; Erkundungs-Deckel auf Kombinationen statt Knoten — None-Fall von >5 min auf 0,37 s; Fall-Welten einmal gebaut + Suche führt beim Probieren strukturell NIE ein schreibendes Werkzeug aus; Quellen-Cache — findende Suche von ~40 s auf 0,38 s). **Als Nächstes: Scheibe C** — die Verdrahtung als Primärpfad (Raster-Absicht → Ziel-Werkzeug + Planfälle als Daten, Deuter liefert die Begriffe, Anker-Leine, Zählwerk, altes Dispatch als sterbliches Netz).

**Grün, neu (2026-07-09, ③ Scheibe C — DER PLANER IST PRIMÄRPFAD, live, e5fdf48):** die Verdrahtung. Die Absicht „beziehung" trägt Ziel-Werkzeug + Planfälle als **Saat-Daten** (`werkzeuge_auskunft.BEZIEHUNG_FAELLE`); der Plan wird daraus **einmal pro Prozess deduziert** (~0,4 s) und gecacht — jede Nachricht zahlt nur Millisekunden. `relate_geplant` = Planer zuerst, **jedes** Scheitern fällt GEZÄHLT aufs handgebaute Netz; **beide Rufer** laufen durch (Deuter-Zelle UND Regex-Erkenner — die Regex erkennt nur noch, rechnet nicht mehr). `genus/zaehlwerk.py` (neu): Treffer/Rückfall/anker_frei je Absicht als fail-silenter JSONL-Puffer (~/.genus/) — die Rohdaten für ④; eine Antwort scheitert NIE an Telemetrie. **Ehrlicher Design-Fund:** die geplante strenge Anker-LEINE hätte die testbewiesene Deuter-Kernfähigkeit getötet, Paraphrasen aufzulösen („wuffwuff" → Hund) — der bestehende Test fing es; Auflösung: die Beziehungs-Antwort ist **selbst-offenlegend** (nennt beide Begriffe in »«), also wird die Leine **Sensor statt Tor** („gedeutet statt zitiert" wird gezählt, nie geblockt; wo eine Antwort ihre Eingaben nicht nennt, darf daraus wieder ein Tor werden). +8 Tests, **1097 grün**. **LIVE auf dem Pi (echtes 755k-Ledger, read-only): der selbst-deduzierte Plan beantwortet „Ist ein Hund ein Tier?" gläsern mit vollem Weg** (Hund → Haushund → Heimtier → Haustier → nichtmenschliches Tier → Tier, Vertrauen 0.50), **Zählwerk: `beziehung: treffer`** — der Planer-Pfad, nicht das Netz. Damit ist der deduktive Planer-Bogen v1 rund: Werkplan (Schritt 1) → liefert (A) → Rückwärts-Suche (B) → Primärpfad (C). **Offen im Bogen:** weitere Absichten aufs selbe Muster ziehen (vergleich/ursache sind formgleich); die Abbau-Entscheidung (Netz stirbt pro Absicht) braucht ④; der Telegram-Bot lädt den neuen Pfad beim nächsten Neustart.

**Grün, neu (2026-07-09, ③ VERGLEICH + URSACHE aufs Primärpfad-Muster, ea6c2a5):** Ronny: „mach die anderen Absichten auch noch." Die Verallgemeinerung — **eine Mechanik, drei Absichten**: `ABSICHT_SAAT` (Absicht → Ziel-Werkzeug + Plan-Eingaben + Planfälle) + `plan_fuer`/`_geplant` tragen alle; `relate_/vergleich_/ursachen_geplant` sind dünne Hüllen. Zwei neue Primitive: **`gemeinsame_kategorien`** (die klassische LCA-Frage als EIN Graph-Schritt — das Urteil der Vergleichsfrage; die Paar-Auflösung läuft über ZWEI verschiedene Auflöser, weil ein Werkzeug je Plan nur einmal vorkommt) und **`ursachen_im_graph`**. Saat-Fälle diversifiziert (Apfel/Birne→Kernobst; Bakterie→Infektion mit Richtungs-Fall), numerische Q-IDs wie in echt (ein synthetisches `Q_bakterie` matchte das Label-Muster nicht — Test fing es). **Klassen-Fund in der Suche:** die alphabetische Quellen-Ordnung hungerte die Aufzählung aus, sobald ein früh-alphabetisches Mehr-Parameter-Werkzeug riesige Unterbäume vor den Auflösern aufspannte → Quellen jetzt nach **Einfachheit** (wenigste Parameter zuerst) — Occam-konsistent, skaliert mit wachsender Registry. +7 Tests, **1104 grün**. **LIVE (echtes Ledger):** „Was haben Apfel und Birne gemeinsam?" → *beide zählen zu »Apfelfrucht« (und weiter zu »Obst« und »Kernobst«)*; „Was verursacht eine Infektion?" → *»Krankheitsauslöser«, »Krankheitserreger«, »Krankheitserregerübertragung«* — **beide über den Plan-Pfad** (Zählwerk: `vergleich: treffer`, `ursache: treffer`). Damit laufen DREI Absichten Planer-zuerst; handverdrahtet im Antwort-Kern bleiben: der Ja/Nein-Kausal-Zweig, gender, definition (je eigener Schnitt).

**Grün, neu (2026-07-10, PROPOSAL #14 — DIE GEBÄRDEN-SCHNELLSPUR, die selbst-erspürte Emoji-Lücke geschlossen):** Ronny: „gehen wir proposal #14 an." Der Vorschlag war KEIN von uns gestellter — der Fehlgriff-Konsument (Selbst-Codieren Stufe 0) hatte ihn selbst erzeugt: „Ich spüre eine Lücke: Nachrichten, die ich gar nicht deuten konnte — 3-mal in echten Gesprächen (selbst-kalibrierte Schwelle 3)." Vom Pi read-only gelesen (`ExperienceProposal`, `verstehen.unklar=verstehens_luecke`, experience #12); die auslösenden Nachrichten stehen NICHT im Ledger (Membran-Reinheit, CI-Gate) — die Diagnose vom Testabend war „Emoji". **Wurzel:** eine reine Emoji-Nachricht („👍", „🙏") ging ans Deuter-Modell, und ein 1.5B-Modell rät bei nacktem Emoji unzuverlässig → `unklar`. GENUS' eigener Plan war „schärfere Few-Shots / stärkeres Modell". **Wir machten es strukturell BESSER:** ein Emoji ist die *deterministischste* Eingabe überhaupt — es verdient dieselbe Behandlung wie die exakten Text-Kommandos, eine gläserne, modellfreie **Schnellspur VOR dem Deuter** (`genus/gebaerde.py`). Nur hoch-eindeutige Gesten (kein Ankreuzzwang: 🤔/😴 fallen ehrlich durch), Abbildung auf bestehende Floskel-Blätter (lob/dank/gruss/kritik — KEINE neue Handler-Fläche, nur ein sicherer Eingang zu den warmen Zellen); reine-Emoji-Erkennung über `isalnum()` (gemischter Text bleibt Sache des Modells), Modifikatoren (VS16/ZWJ/Hautton) abgestreift. **Ehrliche Herkunft:** die Belegung trägt Quelle „gebaerde", nicht „model:deuter"; der `_DEUTED`-Transparenz-Zusatz („vom Sprachmodell gedeutet") entfällt korrekt (kein Modell lief). Der Segment-Loop faktorisiert (`_segmente_loesen`), von Deuter- UND Gebärden-Pfad geteilt. Live-erprobt: 👍→„Danke.", 🙏→„Gern geschehen!", 👋→„Hallo! Schön, dass du da bist.", 👎→„Danke für die Rückmeldung…", 🙏❤→„Gern geschehen! Danke." — der Deuter (das Modell) bleibt dabei **untätig** (getestet). +11 Tests, **1120 grün**. Deploy = reiner `git pull` + Bot-Neustart (keine Saat, kein Schema, keine neue Abhängigkeit). Bleibt: Ronnys Gate — #14 auf dem Pi freigeben (`genus governance`), die Lücke IST jetzt geschlossen.

**Vorgemerkt (Ronny, 2026-07-10, nach dem ersten Planer-Testabend): DIE SEELE DER ANTWORTEN.** „Es fehlt die Seele, nicht nur die Symbole" — der Kontext-Vormerk von unten, jetzt im echten Chat gefühlt. Vier diagnostizierte Struktur-Stellen: die Stimme ist von wortlautfesten Antworten ausgesperrt (rohe Schablonen genau dort); wortgleiche Wiederholung wirkt mechanisch; die Substanz-Komposition (Vertiefung/Anschluss/Notiz) erreicht nur Definitionen; Deuter-Fehlgriffe laufen an den warmen Zellen (lob/dank) vorbei. Startidee für den gemeinsamen Design-Schritt: **„Rahmen frei, Kern fest"** — Fakten-Kern verbatim eingebettet (Schlagzeilen-Muster, Richtung kann nicht kippen), Stimme komponiert frei drumherum. Ausdrücklich EIN DESIGN-SCHRITT IM GESPRÄCH mit Ronny (Zielbild: er schreibt Wunsch-Antworten vor), nicht unilateral. Offene Fäden desselben Abends: Deuter-„ursache"-Übergriff (Zählwerk-Befund #1, 2× anker_frei), Kandidat Absicht „ort" (Kassel/Hessen = part_of, nicht is_a), Proposal #14 am Gate (Emoji-Lücke, selbst erzeugt).
...
**RICHTUNGS-WENDE (Ronny, 2026-07-08): WACHSTUM = KREISLAUF, NICHT MEHR MUSTER.** Nicht den Kern ändern — ändern, *woran* Wachstum gemessen wird und *wodurch* neue Fähigkeiten entstehen. Der Fehler bisher: GENUS wächst zu oft durch neue Spezialfälle (Frage scheitert → Muster/Zelle/Handler/Test) — zuverlässiger in bekannten Formen, aber nicht freier. „Guter Körper, zu enges Nervensystem." Der Pi (755k Events, intakte Seal-Kette) ist ein Schatz — GENUS als **Messmaschine der eigenen Fähigkeitsentwicklung** nutzen, nicht neu beginnen. **Die neue Prioritäts-Reihenfolge (löst die alte „Rot"-Liste ab):** ① Betriebs-Drifts fixen (**Wurzel gefunden: `GENUS_CORE_ID` ungesetzt → doctor WARN + Status-Publish scheitert nächtlich + Anker-Export übersprungen**); ② die Gesprächsschicht modularisieren, `companion.py` entlasten; ③ ein echter **Tool-Planer** (Absicht → Werkzeugwahl → Kern führt deterministisch aus → Antwort geprüft; Regex nur Schnellspur, nicht Wachstumspfad) — **bleibt Organ, kein Orakel** (komponiert aus der Werkzeug-Registry, GBNF-gegrenzt, graph-verifiziert); ④ ein erstes **Skill-Dashboard** (Lücken, Fähigkeiten, Tests, Parser-/Planer-/Werkzeug-Trefferquoten, Regression) — **Thermometer, kein Ziel**: es muss GENERALISIERUNG messen (neue Absichten), sonst belohnt es genau die Spezialfall-Falle; ⑤ Selbst-Codieren härter (Pfadgrenzen, echte Abnahmeverträge, kein „bestanden"-Schein durch Formaltests, Erfolg NACH dem Merge gemessen). Der geschlossene Kreis: Lücke → Plan → Werkzeug/Fähigkeit vorschlagen → testen → Mensch gibt frei → **live messen → aus Erfolg/Misserfolg kalibrieren** (das offene Ende; das Dashboard IST der fehlende Sensor). Siehe Gedächtnis `wachstums-doktrin-kreislauf-statt-muster`.

**Rot — abgelöst (Alt-Liste, teils noch gültig als Detail-Aufgaben unter der neuen Richtung):** bessere Plan-Finder-Kandidaten; Embedder-Cosine-Nudge; Kurvendiskussion erweitern; `integrity`-Register (nicht dringend). Parallel weiter gültig: Selbst-Codieren Stufe 1 (der Bauer wird selbst ein registriertes Werkzeug, dann kann ein Proposal aus Stufe 0 direkt einen Aufruf des Bauers vorschlagen); der erste echte Anwendungsfall für ein Rezept (Kurvendiskussion, komponiert aus den vier Mathe-Werkzeugen). Und weiterhin: die Identitätsfrage („Weißt du, dass du GENUS bist?") hat noch keine gute Heimat in `frage-genus` (fällt heute ehrlich, aber ungenau auf `ursache`/`frage-begriff` durch) — ein Blatt „identität" ist ein guter Kandidat; vollständigere Segmentierung (das Modell übersieht gelegentlich ein Segment in einer Drei-Akt-Nachricht); Treffer-Quote-Kennzahl aus Folge-Signalen (lernt, WELCHE Bausteine je Zelle nützen); Raster-Inquiries (GENUS fragt bei Unsicherheit nach); Plural-Morphologie beim Objekt einer Beziehungsfrage; Phase D (früh); Korrelation≠Kausalität; Ziel-Objekt; Punkt ③ Fachwissen-INHALT (Infrastruktur bereit, wartet auf Ronnys Domänen-Wahl + ggf. eine Prüfungsordnungs-Quelle) und der Rest von Punkt ④ (Tagespuffer + Nacht-Konsolidierung + Morgen-Push — die erste echte PUSH-Fähigkeit der Membran, verdient denselben sorgfältigen Design-Schritt wie das Gedächtnis-Konzept selbst, nicht unilateral gebaut). Das LLM bleibt am Rand, gedeckelt.
