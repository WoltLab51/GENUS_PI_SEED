...
def _record_network_belief(conn, check_event_id: int, status: str) -> dict:
    ...
    active = projection.active_belief(conn, CLAIM_NETWORK)
    if active is None:
        ...
        if claim_value == UNSTABLE:
            _record_network_proposal(conn, belief_id, check_event_id)
        return {
    ...
    event_payload = {
        ...
        "derivation": DERIVATION_CHECK,
        "supporting_events": [check_event_id],
        "reason": f"{CLAIM_NETWORK}_changed_to_{claim_value}",
    }
    ...
    if claim_value == UNSTABLE:
        _record_network_proposal(conn, new_belief_id, check_event_id)
    return {
